...
for (const slug of fs.readdirSync(path.join(ROOT, "products"))) {
  const queueExample = path.join(ROOT, "products", slug, "queue", "example.json");
  if (fs.existsSync(queueExample)) {
    const q = JSON.parse(fs.readFileSync(queueExample, "utf8"));
    q.date = date;
  ...
  const auditExample = path.join(ROOT, "products", slug, "audits", "example.json");
  if (fs.existsSync(auditExample)) {
    const a = JSON.parse(fs.readFileSync(auditExample, "utf8"));
    a.date = date;
    const out = path.join(ROOT, "products", slug, "audits", date + ".json");
    fs.writeFileSync(out, JSON.stringify(a, null, 2) + "\n");
    n++;
    console.log("  seeded " + path.relative(ROOT, out));
  }
}
console.log("\n  " + n + " file" + (n === 1 ? "" : "s") + " seeded for " + date + "\n");
